...
class UnisenderApi
{
    ...
    {
        if ($apiKey = \Arr::get($config, 'api_key')) {
            $this->apiKey = $apiKey;
        }

        if ($encoding = \Arr::get($config, 'encoding')) {
            $this->encoding = $encoding;
        }

        if ($retryCount = \Arr::get($config, 'retry_count')) {
        ...
        if ($compression = \Arr::get($config, 'compression')) {
            $this->compression = $compression;
        }

        if ($platform = \Arr::get($config, 'platform')) {
            $this->platform = $platform;
        }

        if ($lang = \Arr::get($config, 'lang')) {
            $this->lang = $lang;
        }

        if ($this->apiKey = \Arr::get($config, 'api_key')) {
            $this->encoding = $encoding;
        }
    ...
    {
        $this->lang = in_array($language, static::$languages, true)
            ? $language
